...
class Cv_HeapStall :  public Cv_Minion< ParentStall>, public Cv_Shared< MThrd>, public Cv_PtrIteratorImpl< Cv_HeapStall< Arena, ParentStall, TValueType, SzBits >>, public Arena::Janitor
{
public:
	...
	typedef Cv_Shared< MThrd>	BaseShared;
    enum 
    { 
    ...
    Cv_Atomic< uint16_t, MThrd>     m_Info;                             // 15-bit for offset in Parent and 1-bit for clean status; Max of 32K addresses support
...
template < class X>
    const X     &AccAt(uint32_t i) const { return static_cast< X*>( ( void *) &m_Page[ 0])[i]; }

    template < class X>
    X           *PtrAt(uint32_t i) { return &static_cast< X*>( ( void *) &m_Page[ 0])[i]; }

template < class X>
    void        SetAt(uint32_t i, const X &x) { static_cast< X*>( ( void *) &m_Page[ 0])[i] = x; }
...
public:
    ...
    void    DetachChild( Arena *arena, LeafStall *st)                                       // detach a child stall      
    {
...
public:
    ...
    typedef typename  RootStall::LeafStall  LeafStall;
...
public:
    ...
    void    DetachChild( Arena *arena, RootStall *spot)  {  m_TopStall = nullptr; }
...
template < uint32_t PageSz>
    ...
    void    ReloadStall( LeafStall *spot, uint32_t pageSz)
    {
    };

    void    ScrubStall( LeafStall *spot, uint32_t sz)
    {
        return;
    }
};
...
public:
    ...
    void    ScrubStall( LeafStall *spot, uint32_t pageSz)
    {
